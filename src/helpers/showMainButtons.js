...
const showMainButtons = async (ctx, mainText) => {
  try {
    ...
    const text = registered
      ? "Ты зарегистрирован на еженедельные встречи. Ожидай приглашения."
    ...
    await ctx.reply(
      "Произошла ошибка при загрузке пользовательских данных. Попробуй ещё раз открыть меню с помощью /menu"
    );
    logError(error, ctx);
  }
};
